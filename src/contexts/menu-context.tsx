...
import { getMenuItems, addDishToMenu, updateMenuItem, deleteMenuItem, getCalendarData, Dish, MenuItem } from '@/lib/api';
...
interface MenuContextType {
  ...
  updateDish: (id: string, updates: { servings?: number; notes?: string }) => Promise<void>;
...
export function MenuProvider({ 
  ...
  const addDish = async (dishId: string, servings: number, notes?: string) => {
    try {
      const newMenuItem = await addDishToMenu(dishId, selectedDate, servings, notes);
  ...
  // Update a dish
  const updateDish = async (id: string, updates: { servings?: number; notes?: string }) => {
    try {
      ...
      setDishes(prev => prev.map(dish => 
        dish.id === id ? updatedMenuItem : dish
      ));
    } catch (err) {
  ...
  const removeDish = async (id: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to remove dish';
      setError(errorMessage);
  ...
  // Get calendar data
  const getCalendarDataForRange = async (startDate: string, endDate: string) => {
    try {
