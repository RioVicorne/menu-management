...
export default function StatsTab({
  ...
  monthEvents: StatsEvent[];
  yearEvents: StatsEvent[];
  currentDate: Date;
  topDishes: { name: string; count: number }[];
  topIngredients: { name: string; count: number }[];
}) {
  const weekly = React.useMemo(
    () => buildWeeklyBuckets(monthEvents, currentDate),
    [monthEvents, currentDate]
  );
  const monthly = React.useMemo(
    () => buildMonthlyBuckets(yearEvents, currentDate),
    [yearEvents, currentDate]
  );
  ...
  return (
    <div className="space-y-8">
      <div className="bg-background rounded-xl p-6 shadow-sm border border-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="inline-flex items-center px-2 py-1 text-xs rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">
            Tuần cao nhất: {weekly.labels[weeklyBestIdx]} (
            {weekly.values[weeklyBestIdx]})
          </span>
        </div>
        <div className="text-xs text-muted-foreground mb-4">
          Số món mỗi ngày trong tuần hiện tại
        </div>
        <BarChart labels={weekly.labels} values={weekly.values} />
        <div className="mt-3 text-xs text-muted-foreground">
          Chú thích: Cột xanh là số món trong ngày. Huy hiệu hiển thị ngày cao
          nhất trong tuần.
        </div>
      </div>
      ...
      <div className="bg-background rounded-xl p-6 shadow-sm border border-border">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="inline-flex items-center px-2 py-1 text-xs rounded-full bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300">
            Tháng nhiều nhất: {monthly.labels[monthlyBestIdx]} (
            {monthly.values[monthlyBestIdx]})
          </span>
        </div>
        <div className="text-xs text-muted-foreground mb-4">
          Tổng số món theo tháng trong năm hiện tại
        </div>
        <BarChart labels={monthly.labels} values={monthly.values} />
        <div className="mt-3 text-xs text-muted-foreground">
          Chú thích: Cột xanh là tổng món mỗi tháng. Huy hiệu hiển thị tháng cao
          nhất.
        </div>
      </div>
            ...
            {topDishes.map((d, i) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  <span
                    className={`inline-flex items-center justify-center w-6 h-6 text-xs font-semibold rounded-full ${i === 0 ? "bg-yellow-400 text-yellow-900" : i === 1 ? "bg-gray-300 text-gray-800" : i === 2 ? "bg-amber-600 text-white" : "bg-blue-100 text-blue-700"}`}
                  >
                    {i + 1}
                  </span>
      ...
      <div className="bg-background rounded-xl p-6 shadow-sm border border-border">
        <h3 className="text-lg font-semibold text-foreground mb-4">
          Nguyên liệu dùng nhiều trong tháng
        </h3>
          ...
          <ol className="space-y-2">
            {topIngredients.map((d, i) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  <span
                    className={`inline-flex items-center justify-center w-6 h-6 text-xs font-semibold rounded-full ${i === 0 ? "bg-emerald-400 text-emerald-900" : i === 1 ? "bg-emerald-200 text-emerald-900" : i === 2 ? "bg-teal-400 text-teal-900" : "bg-green-100 text-green-700"}`}
                  >
                    {i + 1}
                  </span>
                  <span className="text-sm text-foreground">{d.name}</span>
