...
export async function getMenuItems(date: string): Promise<MenuItem[]> {
  ...
  try {
    ...
    if (error) {
      console.warn('Error fetching menu items (running in mock mode):', error.message);
    ...
    if (data && data.length > 0) {
      ...
      return data.map(item => ({
        ...item,
        ten_mon_an: dishes.find(d => d.id === item.ma_mon_an)?.ten_mon_an || 'Unknown dish'
      }));
    }
...
export async function getCalendarData(startDate: string, endDate: string): Promise<any[]> {
  if (!supabase) {
    // Return mock data when Supabase is not configured
    return [];
  }
  ...
  try {
    ...
    if (error) {
      console.warn('Error fetching calendar data (running in mock mode):', error.message);
      // Return empty array instead of throwing error in mock mode
      return [];
    }
